...
class KerasImageClassifier:
    def __init__(self):
        ...
        self.class_pub = rospy.Publisher("/predicted_class", String, queue_size=10)
    ...
    def load_labels(self, path):
        with open(path, 'r') as f:
            lines = f.readlines()
            return [line.strip().split(' ', 1)[1] if ' ' in line.strip() else line.strip() for line in lines]
    ...
    def toggle_callback(self, msg):
        ...
        rospy.loginfo(f"Captura automàtica {state}")

    def image_callback(self, msg):
        try:
            # Convertir imagen
            cv_image = self.bridge.imgmsg_to_cv2(msg, desired_encoding="bgr8")
            resized_for_model = cv2.resize(cv_image, self.input_shape) # Imatge per al model
            
            img_normalized = resized_for_model.astype(np.float32) / 255.0
...
if __name__ == "__main__":
    rospy.init_node("keras_detector_node_2")
    try:
        KerasImageClassifier()
        rospy.spin()
    except rospy.ROSInterruptException:
        rospy.loginfo("Node keras_detector_node parado.")
    except Exception as e:
        rospy.logerr(f"Excepción no controlada en keras_detector_node: {e}")